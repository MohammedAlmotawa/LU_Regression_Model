...
List = ["%.2d" % i for i in range(32)] #List ['01','02',...]
ListInt = [int(z) for z in List] #Interger List [01, 02,...]
ListMonth = ["%.2d" % i for i in range(13)]
ListMonthInt = [int(q) for q in ListMonth]

Month1 = NE.create_sheet ('Day1')
Day2 = NE.create_sheet ('Day2')
for row in range (9,50):
    for month in range(len(ListInt)):
        NE.create_sheet (str(month))
        for day in range (len(ListInt)):
            char = get_column_letter (1)
            cell = char + str (row)
            x = str(sheet[cell].value)
            if x.startswith("'2019-"+str(month)+"-"+str(day)):
                Month(str(month))[cell] = x
            break
        
        

NE.save ('DividedByMonth.xlsx')
